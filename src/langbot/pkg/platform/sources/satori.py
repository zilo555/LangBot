...
class SatoriMessageConverter(abstract_platform_adapter.AbstractMessageConverter):
    ...
    async def yiri2target(
        ...
        for component in message_chain:
            if isinstance(component, platform_message.Plain):
                        ...
                        try:
                            ...
                            if uploaded_url:
                                ...
                                content_parts.append(
                                    f'<img src="data:{mime_type};base64,{raw_b64}"/>'
                                )
                ...
                if hasattr(component, "url") and component.url:
                    content_parts.append(
                        f'<file url="{component.url}" name="{getattr(component, "name", "")}"/>'
                    )
    ...
    async def target2yiri(
                ...
                for pattern, msg_type in patterns:
                    ...
                    if sub_match:
                        ...
                        if msg_type == "image":
                            ...
                            components.append(
                                platform_message.At(target=str(target_id))
                            )
                            ...
                            components.append(
                                platform_message.Quote(
                                    message_id=str(quote_id), origin=[]
                                )
                            )
                            ...
                            components.append(
                                platform_message.File(url=video_url, name="video")
                            )
                        elif msg_type == "file":
                            file_url = sub_match.group(1)
                            # 尝试从标签中提取文件名
                            name_match = re.search(
                                r'name=["\']([^"\']*)["\']', match_text, re.IGNORECASE
                            )
                            file_name = name_match.group(1) if name_match else ""
                            components.append(
                                platform_message.File(url=file_url, name=file_name)
                            )
                        break
        ...
        await adapter.logger.debug(
            f"Satori 消息解析完成: 共 {len(components)} 个组件 内容长度={len(content)} 字符"
        )
    ...
    async def target2yiri(
        ...
        if event_type == "message-created":
            ...
            message_chain.insert(
                0, platform_message.Source(id=message_id, time=datetime.datetime.now())
            )
                ...
                try:
                    if isinstance(msg_timestamp, (int, float)):
                        event_time = (
                            ...
                            else int(msg_timestamp * 1000)
                        )
                        event_time = (
                            event_time // 1000 if event_time > 1e12 else event_time
                        )
                    else:
                        # Try parsing ISO format
                        event_time = int(
                            datetime.datetime.fromisoformat(
                                str(msg_timestamp).replace("Z", "+00:00")
                            ).timestamp()
                        )
            ...
            if is_private:
                ...
                await adapter.logger.info(
                    f"Satori 私聊消息已构建: 用户ID={user_id}, 用户名={user_name}"
                )
                ...
                group_name = (
                    guild.get("name", "Unknown Group") if guild else "Unknown Group"
                )
                ...
                await adapter.logger.info(
                    f"Satori 群消息已构建: 群ID={group_id}, 发送者={user_name}"
                )
                ...
                await adapter.logger.info(
                    f"Satori 私聊消息已构建 (fallback): 用户ID={user_id}, 用户名={user_name}"
                )
                return friend_message
        return None
...
class SatoriAdapter(abstract_platform_adapter.AbstractMessagePlatformAdapter):
    ...
    heartbeat_task: typing.Optional[asyncio.Task] = pydantic.Field(
        exclude=True, default=None
    )
    ...
    async def handle_message(self, message: str):
        ...
        try:
            ...
            await self.logger.error(
                f"解析 WebSocket 消息失败: {e}, 消息内容: {message}"
            )
        except Exception as e:
        ...
        try:
            ...
            if is_private:
                ...
                await self.logger.warning(
                    f"Satori 无法确定消息类型，使用私聊作为fallback: channel_type={channel_type}"
                )
                sender = platform_entities.Friend(
                    id=user_id,
                    nickname=user_name,
        ...
        try:
            async with self.session.request(
                ...
                if response.status == 200:
                    ...
                    await self.logger.error(
                        f"Satori API 请求失败: {response.status} - {text}"
                    )
                    return None
        except Exception as e:
    ...
    async def upload_image(
        ...
        try:
            ...
            form_data.add_field(
                "file", image_bytes, filename=f"image.{ext}", content_type=mime_type
            )
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    if isinstance(result, dict) and "url" in result:
                        ...
                        await self.logger.warning(
                            f"Satori 图片上传响应格式未知: {result}"
                        )
                        return None
                else:
                    text = await response.text()
                    await self.logger.error(
                        f"Satori 图片上传失败: {response.status} - {text}"
                    )
                    return None
        except Exception as e:
    ...
    async def reply_message(
        ...
        try:
            ...
            channel_id = ""
            if (
                hasattr(message_source, "source_platform_object")
                and message_source.source_platform_object
            ):
            ...
            if not channel_id:
                if isinstance(message_source, platform_events.GroupMessage):
                    ...
                    if hasattr(message_source.sender, "group") and hasattr(
            ...
            # Last fallback
            if not channel_id:
                if hasattr(message_source, "sender") and hasattr(
