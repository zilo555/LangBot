...
with engine.begin() as conn:
    ...
    while True:
        row = conn.execute(
            ...
            .where(AgentRun.__table__.c.status == 'queued')
            .order_by(AgentRun.__table__.c.priority.desc(), AgentRun.__table__.c.id.asc())
            .limit(1)
        ).mappings().first()
        if row is None:
            break
        runtime_id = f'runtime-{len(claimed) % runtime_count}'
        conn.execute(
            update(AgentRun.__table__)
            .where(AgentRun.__table__.c.run_id == row['run_id'])
            .values(status='completed', claimed_by_runtime_id=runtime_id, dispatch_attempts=1)
        )
        claimed.append(row['run_id'])
    rows = conn.execute(select(AgentRun.__table__)).mappings().all()

assert len(claimed) == run_count
assert len(set(claimed)) == run_count
assert all(row['status'] == 'completed' for row in rows)
assert all(row['dispatch_attempts'] == 1 for row in rows)
assert claimed[0] == 'run-000'
assert claimed[-1] == 'run-099'
print(f'LEDGER_STRESS_OK runs={run_count} runtimes={runtime_count}')
`;
  ...
  try {
    if (!existsSync(langbotRepo)) {
      ...
      if (proc.error) {
        ...
        result.status = "fail";
        result.reason = `ledger stress exited with status ${proc.status}`;
      }
    }
  } catch (error) {
    result.status = "fail";
